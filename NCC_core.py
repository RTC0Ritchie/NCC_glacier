...
class img2vxy_first:
        ...
        for ii in tqdm(range(self.lengthxy[0])):
            for jj in range(self.lengthxy[1]):
                ...
                if self.type == 'O-NCC-norm' or self.type == 'O-NCC-sig':
                    imgker = self.imgs.direction_old[self.startxy[0]+ii-self.ker_size//2:self.startxy[0]+ii+self.ker_size//2+1,
                                                     self.startxy[1]+jj-self.ker_size//2:self.startxy[1]+jj+self.ker_size//2+1]
                    imgwin = self.imgs.direction_new[self.startxy[0]+ii-self.win_size//2:self.startxy[0]+ii+self.win_size//2+1,
                                                     self.startxy[1]+jj-self.win_size//2:self.startxy[1]+jj+self.win_size//2+1]
                else:
                    imgker = self.imgs.img_old[self.startxy[0]+ii-self.ker_size//2:self.startxy[0]+ii+self.ker_size//2+1,
                                               self.startxy[1]+jj-self.ker_size//2:self.startxy[1]+jj+self.ker_size//2+1]
                    imgwin = self.imgs.img_new[self.startxy[0]+ii-self.win_size//2:self.startxy[0]+ii+self.win_size//2+1,
                                               self.startxy[1]+jj-self.win_size//2:self.startxy[1]+jj+self.win_size//2+1]
    ...
    def completeprocess(self,is_after_err=False,past_arr_urf=None,mask=None):
        ...
        try:
            ...
            print('Completed!')
            self.delta_x += (-self.win_size // 2 + self.ker_size // 2+1)
            self.delta_y += (-self.win_size // 2 + self.ker_size // 2+1)
...
class process_all:
    ...
    def finalize(self,v_str,day):
        ...
        if self.stride > 1:
            ...
            df = pd.DataFrame(tmpvx)
            df.to_csv(self.target_file + '/' + self.file_name[0] + v_str + '.csv', index=False)

            df = pd.DataFrame(tmpvy)
            df.to_csv(self.target_file + '/' + self.file_name[1] + v_str + '.csv', index=False)
